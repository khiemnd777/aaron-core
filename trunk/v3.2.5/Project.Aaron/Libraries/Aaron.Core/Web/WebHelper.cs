...
namespace Aaron.Core.Web
{
    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        public virtual string GetThisPageUrl(bool includeQueryString, bool useSsl)
        {
            ...
            {
                string webHost = GetWebHost(useSsl);
                if (webHost.EndsWith("/"))
                    webHost = webHost.Substring(0, webHost.Length - 1);
                url = webHost + _httpContext.Request.RawUrl;
            }
            else
            {
        ...
        /// <summary>
        /// Gets web host location
        /// </summary>
        /// <param name="useSsl">Use SSL</param>
        /// <returns>Web host location</returns>
        public virtual string GetWebHost(bool useSsl)
        {
            ...
            else
            {
                ...
                //so let's resolve WebInformationSettings here.
                //Do not inject it via contructor because it'll break the installation (settings are not available at that moment)
                var webSettings = IoC.Resolve<WebInformationSettings>();
                result = webSettings.WebUrl;
            }
            if (!result.EndsWith("/"))
                result += "/";
            if (useSsl)
            ...
            else
            {
                if (SslEnabled())
                {
                    ...
                    if (!String.IsNullOrEmpty(sharedSslUrl))
                    {
                        ...
                        /* we need to set a web URL here (IoC.Resolve<ISettingManager>().WebUrl property)
                         * but we cannot reference Aaron.BusinessLogic.dll assembly.
                         * So we are using one more app config settings - <add key="NonSharedSSLUrl" value="http://www.yourWeb.com" />
                         */
        ...
        /// Gets web location
        /// </summary>
        /// <returns>Web location</returns>
        public virtual string GetWebLocation()
        {
            bool useSsl = IsCurrentConnectionSecured();
            return GetWebLocation(useSsl);
        }
        ...
        /// <param name="useSsl">Use SSL</param>
        /// <returns>Web location</returns>
        public virtual string GetWebLocation(bool useSsl)
        {
            //return HostingEnvironment.ApplicationVirtualPath;

            string result = GetWebHost(useSsl);
            if (result.EndsWith("/"))
        ...
        public virtual void RestartAppDomain(bool makeRedirect = false, string redirectUrl = "")
        {
            ...
            else
            {
                ...
                if (!success)
                {
                    throw new AaronException("Aaron needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
        ...
        private bool TryWriteGlobalAsax()
        {
            try
            {
                //When a new plugin is dropped in the Plugins folder and is installed into AaronCommerce, 
                ...
                //The issue is described here: http://www.nopcommerce.com/boards/t/10969/Aaron-20-plugin.aspx?p=4#51318
        ...
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContext.Items["Aaron.IsPOSTBeingDone"] == null)
                    return false;
                return Convert.ToBoolean(_httpContext.Items["Aaron.IsPOSTBeingDone"]);
            }
            set
            {
                _httpContext.Items["Aaron.IsPOSTBeingDone"] = value;
            }
        }
    }
}
