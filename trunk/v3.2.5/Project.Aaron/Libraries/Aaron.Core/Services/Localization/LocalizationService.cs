...
using Aaron.Data;
//using Aaron.Services.Events;
//using Aaron.Services.Logging;
...
namespace Aaron.Core.Services.Localization
{
    ...
    {
        #region Constants
        private const string LOCALSTRINGRESOURCES_ALL_KEY = "Aaron.lsr.all-{0}";
        private const string LOCALSTRINGRESOURCES_BY_RESOURCENAME_KEY = "Aaron.lsr.{0}-{1}";
        private const string LOCALSTRINGRESOURCES_PATTERN_KEY = "Aaron.lsr.";
        #endregion
        ...
        private readonly UtilitySettings _utilitySettings;
        ...
        {
            if (_currentActivity.CurrentLanguage != null)
                return GetLocaleStringResourceByName(resourceName, _currentActivity.CurrentLanguage.Id);
        ...
        {
            if (_currentActivity.CurrentLanguage != null)
                return GetResource(resourceKey, _currentActivity.CurrentLanguage.Id);
        ...
        public virtual void ImportResourcesFromXml(Language language, string xml)
        {
            ...
            if (String.IsNullOrEmpty(xml))
                return;
            if (_utilitySettings.UseStoredProceduresIfSupported && _dataProvider.StoredProceduredSupported)
            {
                ...
                _context.ExecuteSqlCommand("EXEC [LanguagePackImport] @LanguageId, @XmlPackage", 600, pLanguageId, pXmlPackage);
            }
            else
            {
