...
namespace Aaron.Core.Services.Logging
{
    ...
    {
        #region Constants
        private const string ACTIVITYTYPE_ALL_KEY = "Aaron.activitytype.all";
        private const string ACTIVITYTYPE_BY_ID_KEY = "Aaron.activitytype.id-{0}";
        private const string ACTIVITYTYPE_PATTERN_KEY = "Aaron.activitytype.";
        #endregion
        ...
        {
            return InsertActivity(systemKeyword, comment, _currentActivity.CurrentAccount, commentParams);
        }
        ...
        /// </summary>
        /// <param name="createdOnFrom">Log item creation from; null to load all accounts</param>
        /// <param name="createdOnTo">Log item creation to; null to load all accounts</param>
        /// <param name="accountId">Account identifier; null to load all accounts</param>
        /// <param name="activityLogTypeId">Activity log type identifier</param>
        ...
        public virtual IPagedList<ActivityLog> GetAllActivities(DateTime? createdOnFrom,
            DateTime? createdOnTo, int? accountId, int activityLogTypeId,
            int pageIndex, int pageSize)
        {
            ...
            if (accountId.HasValue)
                query = query.Where(al => accountId.Value == al.AccountId);
